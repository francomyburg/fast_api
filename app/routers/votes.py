...
def vote(vote:schemas.Vote,db: Session = Depends(database.get_db),user: int = Depends(oauth2.get_current_user)):
    ...
    if vote.vot_dir==1:
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail=f"user {user.email} has already vote on post {vote.post_id}")
        ...
        return {"message":"your vote was deleted"}