...
def create_post(post: Post_model,db: Session = Depends(get_db)):
...
@app.put("/posts/{id}")
def update_post(id: int, post: Post_model,db: Session = Depends(get_db)):
    ...
    return {"Updated post":update_post.first()}
